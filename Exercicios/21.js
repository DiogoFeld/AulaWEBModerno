...
// mínimo de cédulas para um determinado valor informado pelo usuário considerando notas
// de R$ 100, R$ 50, R$ 10 e R$ 5 e R$ 1. Seu programa deve mostrar apenas as notas utilizadas.
...
function caixaEletronico(valor) {
    ...
    for (let i in valores) {
        if (valor > valores[i]) {
            quantidade = parseInt(valor / valores[i]);
            valor = valor - (quantidade * valores[i]);
            total = `${total} ${quantidade} nota(s) de R$${valores[i]},`
        }
    }
    return total;
}

console.log(caixaEletronico(148))